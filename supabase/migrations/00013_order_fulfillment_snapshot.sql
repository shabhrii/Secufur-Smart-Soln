-- ==========================================
-- ORDER FULFILLMENT DATA SNAPSHOTTING
-- Option C: Denormalizing buyer data for sellers
-- ==========================================

-- 1. Add immutable snapshot columns to orders
...
-- 2. Update the RPC function to capture these snapshots at creation time
...
BEGIN
  ...
  -- Capture buyer info
  SELECT email INTO v_buyer_email FROM auth.users WHERE id = v_user_id;
  SELECT full_name, phone INTO v_buyer_name, v_buyer_phone FROM public.profiles WHERE id = v_user_id;

  -- 2. Verify address belongs to the user and snapshot it
  ...
  -- 6. Insert Order WITH snapshot data
  INSERT INTO public.orders (
    user_id, address_id, order_status, payment_status, subtotal, shipping_cost, total_amount,
    buyer_name, buyer_email, buyer_phone, shipping_address_json
  ) VALUES (
    v_user_id, p_address_id, 'pending', 'pending', v_subtotal, v_shipping_cost, v_total,
    v_buyer_name, v_buyer_email, COALESCE(v_buyer_phone, v_address.phone), row_to_json(v_address)::jsonb
